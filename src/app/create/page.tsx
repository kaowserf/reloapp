...
function ToolCard({ tool }: { tool: Tool }) {
  return (
    <Link href={`/create/${tool.slug}`} className="group relative block aspect-[4/5] overflow-hidden rounded-3xl border border-white/10 shadow-xl transition-all duration-300 hover:-translate-y-1.5 hover:border-amber-400/40 hover:shadow-2xl hover:shadow-red-900/30">
      <Image src={tool.poster} alt={tool.title} fill sizes="(max-width:640px) 100vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-105" />
      ...
      <div className="absolute inset-x-0 bottom-0 p-6">
        ...
        <h3 className="font-display text-2xl font-bold tracking-tight">{tool.title}</h3>
        <p className="mt-1 text-sm text-white/70">{tool.tagline}</p>
